<template>
  <div class="live">
    <VideoPlayer v-if="videoOptions.sources[0].src !== ''" :options="videoOptions" />
    <div class="m-2">
      <h4 class="mt-4"><b>{{ title }}</b></h4>
      <hr>
      <p><b>{{ username }}</b></p>
      <hr>
      <p>作成日: {{ postdate }}</p>
  ...
  mounted() {
    axios
      ...
      .then((res) => {
        ...
        this.videoOptions.sources[0].src = "https://live.gamma410.win/hls/" + this.streamingkey + ".m3u8";
      })
      .catch((err) => {
        this.posts = err;
      });
